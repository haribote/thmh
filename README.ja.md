...
- **`@thmh/core`**：型とバリアント定義を静的解析し、`catalog.json` を生成するエンジンです。他の2パッケージはこれに依存しています。
- **`@thmh/vite`**：Vite プラグインです。`vite.config.ts` に追加すると、アプリ本体の dev サーバー上で `/__thmh/` にカタログを配信します。
- **`@thmh/cli`**：ホストアプリを持たないリポジトリ（デザインシステム専用リポジトリなど）向けのスタンドアロン CLI です。パッケージ名は `@thmh/cli` ですが、実行コマンドは `thmh` のままです（経緯は #6 を参照してください）。

## npm からのインストール

Vite の dev サーバーを持つアプリであれば、次のようにインストールします。

```bash
npm install -D @thmh/vite@next
```

ホストアプリを持たないリポジトリでは、次のようにインストールします。
...
現時点では `@next` の指定が必須です。
各パッケージの初回公開時に、npm の仕様により `latest` タグにも名前予約用のプレースホルダー `0.0.0` が入ってしまい、安定版をリリースするまではそれが更新されないためです。

## 動かしてみる（プロトタイプ版）

ここでは、現時点のプロトタイプを動かす手順を説明します。
`thmh` CLI は `build` だけを実装しており、`dev` / `init` / `mcp` は「not implemented」というメッセージを返します。
