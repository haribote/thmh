---
id: MCP000 # replace 000 with the next free number in this domain
title:
depends_on: []
used_by: []
# layer: foundation | feature | integration   (optional)
# status: draft | stable                       (optional)
---

# <title>

## Overview

<!-- What this feature is, in one or two sentences. -->

## Requirements

<!-- Link to the requirement in ../requirements.md that this satisfies, and quote the bullet. Append this feature's ID to that bullet in requirements.md. See ../design.md. -->

## Design

<!-- How it works. Add a mermaid diagram when the shape is the point; see ../design.md. -->

## Notes

<!-- Edge cases, open questions, alternatives considered. -->
