...
**What the plugin requires of its host is declared, not assumed.** Vite is a peer dependency because the plugin is one. React and React DOM are peer dependencies because the preview entry renders with them ([UIX001](../ui/UIX001_preview-sandbox.md)), so a host without them cannot serve a preview at all. Declaring them makes a missing one a warning at install time rather than a broken frame at open time.
